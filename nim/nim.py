...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        # if q is None return 0 else return the value of q
        if q == None:
            return 0
        return q
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        new_q += self.alpha * ((reward + future_rewards) - old_q)
    ...
    def best_future_reward(self, state):
        ...
        #else select the maximum value from all the possible rewards
        values = []
        for action in Nim.available_actions(state):
            val = self.q.get((tuple(state),action))
            if val == None:
                val = 0
            values.append(val)
        return max(values)
